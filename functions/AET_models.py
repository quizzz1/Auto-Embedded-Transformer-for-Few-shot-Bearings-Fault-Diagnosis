...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def call(self, q, k, v, training):
        ...
        q = self.q_linear_mapping(
            q, training=training
        )  #  (batch_size, seq_len, d_model)
        k = self.k_linear_mapping(
            k, training=training
        )  #  (batch_size, seq_len, d_model)
        v = self.v_linear_mapping(
            v, training=training
        )  #  (batch_size, seq_len, d_model)
        ...
        output = self.out_linear_mapping(
...
if __name__ == "__main__":
    ...
    print(
        "sample_encoder_layer_output.shape ", sample_encoder_layer_output.shape
    )  # (batch_size, input_seq_len, d_model)
    print("sample_encoder_layer_aw.shape ", sample_encoder_layer_aw.shape)


def get_Linear_Embedding1D(
    input_shape=(2048, 1),
    patch_size=(32,),
    d_model=64,
):
...
def get_TinyRN(
    ...
    x2 = tf.keras.layers.LayerNormalization(
        axis=-1,
        epsilon=1e-6,
    )(x2)
...
class AutoEmbeddingTransformer(tf.keras.Model):
    ...
    def __init__(
        ...
        if cls_token:
            ...
            self.get_cls_token = tf.keras.layers.Dense(
                self.d_model, activation="sigmoid", use_bias=False, name="get_cls_token"
            )
        ...
        self.drop_before_encod = tf.keras.layers.Dropout(
            rate, name="Drop_before_encod_1"
        )
        ...
        """特征 标准化"""
        self.feature_norm = tf.keras.layers.LayerNormalization(
            axis=-1, epsilon=1e-6, name="feature_norm"
        )
    ...
    def call(self, x, training, need_w=False):
        ...
        if self.cls_token:
            ...
            xe = tf.concat(
                values=(cls_token, xe), axis=1
            )  # [bs, 1, dm] concat [bs, pos, dm]
        ...
        if self.positional_encoding_way == "trainable":
            ...
            pe = tf.reshape(
                tensor=pe, shape=(-1, self.num_positions, self.d_model)
            )  # [bs, pos, d_model]
        ...
        x = (
            xe * 0.8 + pe * 0.2
        )  # x的权重为0.8，positional encoding的权重为0.2，弱化编码的影响，强化对数据特征的学习
        x = self.drop_before_encod(x, training=training)
        for i in range(self.num_layers):
            # print('x1.shape',x1.shape)
            x, aw = self.enc_layers[i](x, training)
            attention_weights[
                "pos_layer_{:d}".format(i)
            ] = aw  # positional attention weights
...
def get_AET(num_layers, need_show=False):
    """自监督方式训练Embedding，"""
    model = AutoEmbeddingTransformer(need_show=True)
    model._name = "AET"
    return model
